...
/** Legacy API: See {@link rustysecrets.wrapped.splitSecret}. */
rustysecrets.generate_shares = function(k, n, secret, mimeType, signShares, cb) {
  rustysecrets.wrapped.splitSecret(k, n, secret, mimeType, signShares, function(err, res) {
    if (err != null) cb(err, null)
    else cb(null, res);
  });
};

/** Legacy API: See {@link rustysecrets.wrapped.recoverSecret}. */
rustysecrets.recover_secret = function(shares, verifySignatures, cb) {
  rustysecrets.wrapped.recoverSecret(shares, verifySignatures, function(err, res) {
    if (err != null) cb(err, null);
    else cb(null, res.secret, res.mimeType);
  });
};

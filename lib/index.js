...
/** Legacy API for Sunder: See {@link rustysecrets.wrapped.splitSecret}. */
rustysecrets.generate_shares = function(k, n, secret, mimeType, cb) {
  rustysecrets.wrapped.splitSecret(k, n, secret, mimeType, true, function(err, res) {
    if (err != null) cb(err, null)
    else cb(null, res);
  });
};

/** Legacy API for Sunder: See {@link rustysecrets.wrapped.recoverSecret}. */
rustysecrets.recover_secret = function(shares, cb) {
  rustysecrets.wrapped.recoverSecret(shares, true, function(err, res) {
    if (err != null) cb(err, null);
